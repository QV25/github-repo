#!/usr/bin/env python3
"""
q07_embed_write_subtasks.py — Writing subtasks (embeddings + geprioriteerde mini LLM refine)

Q13: If you chose “Writing & professional communication”, which sub-tasks do you use?
(multiple answers possible)

Doelen:
1) Per Q+A-paar labels → results/q07_pairs.xlsx  (voor validatie/F1)
2) Donor-samenvatting met significante subtaken → results/answers_q07.xlsx

Alleen donors meenemen die in q06 'Writing & professional communication' hebben.
"""
...
Q06_XLS    = pathlib.Path("results/answers_q06.xlsx")     # bron voor writing-donors
OUT_PAIRS  = pathlib.Path("results/q07_pairs.xlsx")       # per Q+A
OUT_DONORS = pathlib.Path("results/answers_q07.xlsx")     # per donor
...
MAX_LABELS_PER_PROMPT = None          # GEEN cap (Q13 kan meerdere subtaken hebben)

# Donor-level significantie (10% + min 3)
MIN_N     = 3
MIN_SHARE = 0.10
TOP_K     = None

# Refinement (geprioriteerd en gecapped)
REFINE_ENABLED   = True
REFINE_MAX_CALLS = 800
CHAT_MODEL       = os.getenv("CHAT_MODEL", "gpt-5-mini")
REFINE_MAXTOK    = 80

# Payload denoising (géén trunc van instructie; alleen ballast maskeren)
...
def _sanitize_name(name: str) -> str:
    # verwijder "or I did not choose ..." en trailing interpunctie/whitespace
...
def _writing_donors_from_q06() -> set[str]:
    ...
    target = _canon("Writing & professional communication")
...
if not writing_donors:
    print("⚠︎ Geen donors met Writing in q06. Exporteer lege sheets.")
...
CATEGORIES = [str(x) for x in proto_texts]   # 1..6 in deze volgorde
...
# ---------- 2) prompts (alleen writing-donors) ----------
...
# fallback: writing-donors zonder sublabels → Other (writing-related)
all_writing = set(pairs["donor_id"].astype(str).unique())
have_subs   = set(valid["donor_id"].astype(str)) if not valid.empty else set()
no_subs     = sorted(all_writing - have_subs)
if no_subs:
    extra = pd.DataFrame({"donor_id": no_subs, "category": "Other (writing-related)"})
