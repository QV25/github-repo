# -*- coding: utf-8 -*-
"""
00_rebuild_llogs_clean.py — Rebuild Llogs_clean.csv from the latest raw "Survey Data logs (24).csv"

Outputs:
  results/derived/Llogs_clean.csv
  results/derived/L_logs.parquet (best effort)

Notes:
- Robust parsing for Q7–Q11 (bands -> midpoints, canonical labels)
- Q12 families via synonym mapping OR one-hot if already present
- Q13–Q17 subtasks: handles either (a) one multiselect text column per question or (b) multiple one-hot columns
- Subtask breadth per family: subtask_breadth_wri/bra/cod/lan/stu
"""
...
BASE = os.getcwd()
RAW_CANDIDATES = [
    os.path.join(BASE, "data", "raw", "Survey Data logs (24).csv"),
    os.path.join(BASE, "data", "raw", "Survey Data Logs (24).csv"),
    os.path.join(BASE, "Survey Data logs (24).csv"),
    os.path.join(BASE, "Survey Data Logs (24).csv"),
]
...
def find_file(candidates):
    ...
    raise FileNotFoundError(
        "Could not find the raw Llogs CSV. Looked for:\n  - " + "\n  - ".join(candidates)
    )
...
def coerce01(s):
    ...
    m1 = s.isin(["1","true","yes","y","ja"])
    m0 = s.isin(["0","false","no","n","nee",""])
    out = pd.Series(np.nan, index=s.index, dtype=float)
    out[m1] = 1; out[m0] = 0
    out = out.fillna(pd.to_numeric(s, errors="coerce"))
    return out.fillna(0).astype(int)

# band midpoints (as in H5)
def map_q7_band(x):
    s = str(x).lower().strip()
...
def q11_score(code):
    m = {"short_sentence":1, "short_paragraph":2, "multiple_paragraphs":3}
    return m.get(code, np.nan)

# Q12 mapping: robust synonyms -> 6 families
Q12_FAMS = {
    "q12__writing_and_professional_communication": [
        "writing & communication","writing and communication","writing","communication","email","report","wri"
    ],
    "q12__brainstorming_and_personal_ideas_fun": [
        "brainstorming / fun","brainstorming and personal ideas fun","brainstorming","ideas","fun","bra"
    ],
    "q12__coding_programming_help": [
        "coding / programming","coding programming help","coding","programming","code","cod"
    ],
    "q12__language_practice_or_translation": [
        "language / translation","language practice or translation","language","translation","lan"
    ],
    "q12__study_revision_or_exam_prep": [
        "study / exam","study revision or exam prep","study","exam","revision","stu"
    ],
...
def detect_families_from_text(cell: str):
    ...
    for p in parts:
        if "did not choose" in p:  # skip informational
            continue
...
# ---- Q13–Q17 subtask mapping (canonical -> keywords to detect in labels/column names) ---- #
SUBTASKS = {
    "q13": {
...
def detect_subtasks_from_text(cell, qtag):
    """
    Map a multiselect cell (comma/semicolon-separated) to {canon_col:0/1} for a qtag (q13..q17)
    """
...
def detect_subtasks_from_columns(df, qtag):
    """
    Map multiple one-hot columns starting with qtag to canonical {canon_col:0/1}, by matching column names.
    """
    ...
    for canon, keys in SUBTASKS[qtag].items():
        ...
        for c in cols:
            low = norm(c)
            if "not_chosen" in low or "not chosen" in low or "breadth" in low:
                continue
            if any(k in low for k in keys):
                m = np.maximum(m, coerce01(df[c]).values)
        out[f"{qtag}__{canon}"] = m
    # if nothing matched any key, create zeros for completeness
...
def derive_subtasks(df, qtag):
    """
    Decide whether qtag is stored as:
      - one multiselect text column (e.g., 'Q13' or 'q13_subtasks'), or
      - many one-hot cols starting with 'q13'.
    Returns a DataFrame with canonical one-hots and a breadth column.
    """
    # Candidate multiselect columns (exact one per question)
    text_candidates = [c for c in df.columns if re.match(rf"^{qtag}\b", c, flags=re.IGNORECASE)]
    # Prefer an exact column named q13 or q13_subtasks etc. with text-like contents
    text_col = None
    for c in text_candidates:
        # if values contain commas/semicolons or are long strings => likely multiselect cell
...
# Drop stray first data row like "Response ID" if present
first_row = df.iloc[0].astype(str).str.lower().tolist()
if any("response id" in v for v in first_row):
    df = df.iloc[1:].reset_index(drop=True)

# Harmonise column names (lower, underscores)
df.columns = [re.sub(r"\s+","_", c.strip().lower()) for c in df.columns]

out = pd.DataFrame(index=df.index.copy())

# IDs
for idcand in ["donor_id","responseid","response_id","id"]:
...
if out["Q7_mid"].isna().all():
    ...
    if cand is None: raise KeyError("Missing Q7/Q7_band/Q7_mid")
...
if out["Q8_mid"].isna().all():
    ...
    if cand is None: raise KeyError("Missing Q8/Q8_band/Q8_mid")
...
if out["Q9_mid"].isna().all():
    ...
    if cand is None: raise KeyError("Missing Q9/Q9_band/Q9_mid")
    out["Q9_mid"] = df[cand].map(map_q9_band)

# Q10
if "q10" in df.columns: out["Q10"] = df["q10"].map(canonical_q10)
elif "daypart" in df.columns: out["Q10"] = df["daypart"].map(canonical_q10)
else: out["Q10"] = "Anytime throughout the day"
...
if len(present_binaries) == len(Q12_COLS):
    ...
    if text_col is None:
        for c in Q12_COLS:
            out[c] = coerce01(df[c]) if c in df.columns else 0
    else:
        hits = df[text_col].apply(detect_families_from_text)
        for c in Q12_COLS:
            out[c] = hits.apply(lambda d: d.get(c,0)).astype(int)

out["task_breadth_main"] = out[Q12_COLS].sum(axis=1)

# Q13–Q17 subtasks
for qtag in ["q13","q14","q15","q16","q17"]:
    # candidate text column if present: exactly 'q13' etc.
    if qtag in df.columns:
        sub_df = derive_subtasks(df.rename(columns={qtag:qtag}), qtag)
    else:
...
# Save
csv_path = os.path.join(OUT_DIR, "Llogs_clean.csv")
parq_path = os.path.join(OUT_DIR, "L_logs.parquet")
out.to_csv(csv_path, index=False)
try:
    out.to_parquet(parq_path, index=False)
except Exception:
    pass
