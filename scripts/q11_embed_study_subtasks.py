#!/usr/bin/env python3
"""
q11_embed_study_subtasks.py — Study subtasks (embeddings + geprioriteerde mini LLM refine)
"""
...
OUT_PAIRS  = pathlib.Path("results/q11_pairs.xlsx")
OUT_DONORS = pathlib.Path("results/answers_q11.xlsx")
...
def _study_donors_from_q06() -> set[str]:
    ...
    target = _canon("Study revision - exam prep")
...
all_study = set(pairs["donor_id"].astype(str).unique())
have_subs = set(valid["donor_id"].astype(str)) if not valid.empty else set()
no_subs   = sorted(all_study - have_subs)
if no_subs:
    extra = pd.DataFrame({"donor_id": no_subs, "category": "Other (study-related)"})
