...
-------------------------------------------
Sub-tasks binnen *Study revision / exam prep*  (meerdere antwoorden)
...
# 6. donor-filter – alleen donors die Study kozen -----------------------
a6_path = pathlib.Path("results/answers_q06.xlsx")
if a6_path.exists():
    a6 = pd.read_excel(a6_path)
    study_donors = set(
        a6[a6["category"].str.contains("Study revision", na=False)]["donor_id"]
    )
else:
    study_donors = set()
counts = counts[counts["donor_id"].isin(study_donors)]
