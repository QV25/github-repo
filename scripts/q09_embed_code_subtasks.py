#!/usr/bin/env python3
"""
q09_embed_code_subtasks.py — Coding subtasks (embeddings + geprioriteerde mini LLM refine)

Q15: If you chose “Coding / programming help”, what coding sub‑tasks do you use?
...
OUT_PAIRS  = pathlib.Path("results/q09_pairs.xlsx")
OUT_DONORS = pathlib.Path("results/answers_q09.xlsx")
...
MAX_LABELS_PER_PROMPT = None   # GEEN cap (Q15 multi-antwoord)
...
# payload denoising (géén truncation van instructie; zachte cap head+tail)
...
def _coding_donors_from_q06() -> set[str]:
    ...
    target = _canon("Coding - programming help")
    def has_coding(cat: str) -> bool:
        parts = [_canon(x) for x in str(cat).split("|")]
        return target in parts
    mask = a6["category"].astype(str).map(has_coding)
...
# ---------- donors filter ----------
coding_donors = _coding_donors_from_q06()
if not coding_donors:
...
# ---------- prompts (alleen coding-donors) ----------
df = pd.read_json(PARSED, lines=True)
df = df[df["donor_id"].astype(str).isin(coding_donors)].copy()
...
all_coding = set(pairs["donor_id"].astype(str).unique())
have_subs  = set(valid["donor_id"].astype(str)) if not valid.empty else set()
no_subs    = sorted(all_coding - have_subs)
if no_subs:
    extra = pd.DataFrame({"donor_id": no_subs, "category": "Other (coding-related)"})
