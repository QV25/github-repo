#!/usr/bin/env python3
"""
q09_embed_code_subtasks.py — hybride (embeddings + mini LLM refinement)

Analyseert *alleen donors die in q06 'Coding - programming help' kozen*.

Sub-tasks (multi-label):
...
# ---------------------- config ----------------------
# Embedding thresholds + refine-band
THRESH    = 0.35
BAND_LOW  = 0.345
BAND_HIGH = 0.355
...
if not coding_donors:
    print("⚠︎ Geen donors met Coding in q06. Exporteer lege sheet met alleen headers.")
...
# 1) categorieën voor embeddings/labels (uit prototypes)
...
# 7) explode + tellen (per coding-donor)
expl = (
    df.explode("labels")
...
# maak de |-gescheiden string
valid["category"] = valid["labels"].apply(
    lambda lst: "|".join(mapping[i] for i in sorted(lst))
)

# strip 'I did not choose …' als donor óók echte subtaken heeft
valid["category"] = (
    valid["category"]
